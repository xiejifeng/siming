...
public class PersonalServiceImpl implements IPersonalService {
	...
	public Map<String, Object> updateUserInfo(BigInteger userId,Map<String, Object> userInfo) {
        ...
        try {
        	...
        	if(userInfo.containsKey("area")){
        		if(!"".equals(userInfo.get("area"))){
        			sb.append(" ,ue.area='"+userInfo.get("area")+"'");
        		}
        	}
	...
	public int doBrushTopic(Map<String, Object> userInfo,Map<String, Object> topicInfo) {
		...
		String isnew = topicInfo.get("isnew").toString();
		try {
			...
			report.setTopic_name(topicInfo.get("label")==null?"":topicInfo.get("label").toString());
			...
			if(topicData.size()==0){
				if(errorCount==0){
					if(Constant.TOPIC_NEW.equals(isnew)){
							scoreInfo.setIncr_score(30*totalCount);
							scoreInfo.setLast_score(oldScore+30*totalCount);
							lastScore = oldScore+30*totalCount;
					}else{
						scoreInfo.setIncr_score(10*totalCount);
						scoreInfo.setLast_score(oldScore+10*totalCount);
						lastScore = oldScore+10*totalCount;
					}
				}else{
					if(Constant.TOPIC_NEW.equals(isnew)){
						scoreInfo.setIncr_score(errorCount+(totalCount-errorCount)*30);
						scoreInfo.setLast_score(oldScore+errorCount*10+(totalCount-errorCount)*30);
						lastScore = oldScore+errorCount*10+(totalCount-errorCount)*30;
					}else{
						scoreInfo.setIncr_score(errorCount+(totalCount-errorCount)*10);
						scoreInfo.setLast_score(oldScore+(totalCount-errorCount)*10);
						lastScore = oldScore+(totalCount-errorCount)*10;
					}
				}
				this.chatRoomDAO.doExecuteSqlAndNamedQuery("SQL_App_13", new Object[]{lastScore,userInfo.get("userId")});
				this.scoreDAO.save(scoreInfo);
	...
	public List<Map<String, Object>> loadTopicList(String subjectId,String paperid,String userId) {
		List<Map<String, Object>> data  = this.dao.findBySqlAndNamedQuery("SQL_App_23", new Object[]{subjectId,userId});
		return data;
	}
	@Override
	public List<Map<String, Object>> loadTopicList(String subjectId,String paperid) {
		List<Map<String, Object>> data  = this.dao.findBySqlAndNamedQuery("SQL_App_23_1", new Object[]{subjectId});
		return data;
	}
	...
	@Override
	public boolean isExistTopicInReport(Map<String, Object> topicInfo,Map<String, Object> userInfo) {
		List<Map<String, Object>> data = this.dao.findBySqlAndNamedQuery("SQL_App_Topic_1", new Object[]{userInfo.get("userId"),topicInfo.get("subjectId")});
		if(data.size()>0){
			return true;
		}
		return false;
	...
	@Override
	public List<Map<String, Object>> getTotalTodayBySubject(String subJectId,
			BigInteger userId) {
		List<Map<String, Object>> data = this.dao.findBySqlAndNamedQuery("SQL_App_Topic_2", new Object[]{subJectId,userId,DateCommonUtils.getTheDate()+" 00:00:00",DateCommonUtils.getTheDate()+" 23:59:59"});
		return data;
	}
	 ...
	 */
	@Override
	public void updateWechatInfoUserInfo(String userExpandId,
			String accessToken, String refreshToken, String expires_date,
			String openid, String sex, String nickname, String province,
			String headimgurl, String city) {
		// TODO Auto-generated method stub
		userExpandDAO.doExecuteSqlAndNamedQuery("SQL_App_UPDATE_WECHAT_USER_INFO",
				new Object[]{accessToken,refreshToken,expires_date,openid,sex,nickname,province,headimgurl,city,userExpandId});
	}
	...
	public boolean existWechatUser(String openId) {
		// TODO Auto-generated method stub
		UserExpand userExpand = userExpandDAO.findOneBySql("SELECT ue.*, su.username,su.`password`,su.account_non_expired," +
				"su.account_non_locked,su.credentials_non_expired FROM sys_user su,user_expand ue WHERE 1 = 1 AND su.id = ue.userId  AND su.account_non_expired = 0 " +
				" AND (ue.wechat_openid = ?)",new Object[]{openId});

		return userExpand!=null&&userExpand.getWechatOpenid()!=null&&!"".equals(userExpand.getWechatOpenid());
	}
	...
	public Map<String, Object> checkWechatUserInfo(String openId, String ipAddr) {
		// TODO Auto-generated method stub
		Map<String, Object> userInfo = userDAO.findUniqueBySqlAndNamedQuery(
				"SQL_App_SELECT_WECHAT_USER_INFO", new Object[] { openId});
		LoginRecord loginRecord = new LoginRecord();
		BigInteger userId;
		if (userInfo != null) {
			userInfo.put("resultCode", "0");
