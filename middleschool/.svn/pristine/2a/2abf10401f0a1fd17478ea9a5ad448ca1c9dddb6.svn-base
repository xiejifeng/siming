...
(function($) {
	$.fn.beamDialog = function(options) {
		...
		return this.each(function() {
			var obj = $(this);
			modalID = getModalID();
			var tmpHtml = '<div class="modal fade" id="{ID}" role="dialog" aria-hidden="true"><div class="modal_wrapper"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">{title}</h4></div><div class="modal-body">{body}</div><div class="modal-footer">{button}</div></div></div></div></div>';
			...
			var div_btn='<div class="btn-group btn-group-justified" role="group" aria-label="...">';
			for (var i = 0; i < options.otherButtons.length; i++) {
				buttonHtml += '<div class="btn-group" role="group">';
				buttonHtml += '<button buttonIndex="' + i + '" class="' + btnClass + ' btn ' + options.otherButtonStyles[i] + '">' + options.otherButtons[i] + '</button>';
				buttonHtml += '</div>';
			}
			div_btn+=buttonHtml+='</div>';
			//替换模板标记
			tmpHtml = tmpHtml.replace(/{ID}/g, modalID).replace(/{title}/g, options.title).replace(/{body}/g, options.content).replace(/{button}/g, div_btn);
			obj.append(tmpHtml);
