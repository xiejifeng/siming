		...
		for (var i = 0; i < _22.length; i++) {
			...
			for (var j = 0; j < _25.length; j++) {
				if (_25[j][_24.valueField] == v) {
					if(_24.isShowValue){
						s=_25[j][_24.valueField]+_24.valueTextSeparator+_25[j][_24.textField];
					}else{
		...
		inp.bind("blur",function(){
			...
			if(!bl){
				if (opts.isContainValid) {
					var name=cmb.find("input.combo-value").attr("name");
					var comb = $(target).find("input.combobox-f[comboName='"+name + "']");
					comb.val(value);
				}else{
					if(value){
			...
			var _v=_35.attr("value")?_35.attr("value"):_34.attr("value");
			_15(target, _v);
