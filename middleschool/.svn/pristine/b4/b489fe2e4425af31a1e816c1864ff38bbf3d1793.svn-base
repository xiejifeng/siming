...
public class AppScoreController {
    ...
    public String exchange(HttpServletRequest request, HttpServletResponse response,Exchange exchange){
    	//验证提交的数据是否为空
    	if (!isValidateOK(exchange)) {
    		request.setAttribute("failed", -1);
    		try {
				request.getRequestDispatcher("/appScore/isExchange.html?id="+exchange.getProductid()).forward(request, response);
