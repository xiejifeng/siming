...
(function($) {
	...
	function _16(_17) {
		...
		if ($(_17).find(".calendar-menu-month-inner").is(":empty")) {
			...
			var t = $("<table></table>").appendTo(
					$(_17).find(".calendar-menu-month-inner"));
			var idx = 0;
			for ( var i = 0; i < 3; i++) {
				var tr = $("<tr></tr>").appendTo(t);
				for ( var j = 0; j < 4; j++) {
					$("<td class=\"calendar-menu-month\"></td>").html(
							_18.months[idx++]).attr("abbr", idx).appendTo(tr);
				}
			}
			...
			$(_17).find(".calendar-menu-month").hover(function() {
				...
				$(this).removeClass("calendar-menu-hover");
					...
					function() {
						...
						_1a.find(".calendar-selected").removeClass(
								"calendar-selected");
	...
	function _f(_2e) {
		...
		$(_2e).find(".calendar-title span").html(
				_2f.months[_2f.month - 1] + " " + _2f.year);
		...
		t.find("td[abbr^=\"" + _2f.year + "," + _2f.month + "\"]").removeClass(
			...
			if (_38) {
				...
				_38 = $.data(this, "calendar", {
					options : $.extend({}, $.fn.calendar.defaults,
							$.fn.calendar.parseOptions(this), _36)
				});
				_7(this);
			}
