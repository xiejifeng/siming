...
//		  $(window.parent.document.getElementById("home")).attr("src",url);
	...
	var _getDictionaryByCode = function(code,fn){
		...
		$.callServiceAsJsonGet(url,{"code":code}, {
			...
			"done" : function(response){
                ...
                for(var i=0;i<data.length;i++){
                	if(data[i].value=='六年级')
                	html+="<option  selected value='"+data[i].id+"'>"+data[i].value+"</option>";
                	else
	...
	//判断当前是否已经做完三题
	var _checkTopicIsOver = function(subjectid){
	...
	return {
		 ...
		 checkTopicIsOver:  _checkTopicIsOver,
