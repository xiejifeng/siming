...
//	});
	...
	$.ffcsAjax({
		url:web_path+"dictionary/queryItemsByAssociation.srp",
		data:{"code":$("#association_code").val()},
	...
	$("#save").click(function(){
		var flag=$("#remark").validate("isValid") && $("#association").validate("isValid");
		if(!flag){
			$("#remark").focus();
			return;
		}
		...
		$.each(rows,function(i,row){
			var str='{"dicts['+indx+'].category.remark":"'+$("#remark").val()+'","dicts['+indx+'].category.categoryCode":"'+$("#categoryCode").val()+'","dicts['+indx+'].state":"NEW'
				+'","dicts['+indx+'].text":"'+row.text+'","dicts['+indx+'].association":"'+$("#association_code").val()+'","dicts['+indx+'].priority":'+row.priority
		...
		$.each(updateRows,function(i,row){
			var str='{"dicts['+indx+'].category.remark":"'+$("#remark").val()+'","dicts['+indx+'].category.categoryCode":"'+$("#categoryCode").val()+'","dicts['+indx+'].state":"UPDATE'
				+'","dicts['+indx+'].code":"'+row.code+'","dicts['+indx+'].text":"'+row.text+'","dicts['+indx+'].association":"'+$("#association_code").val()+'","dicts['+indx+'].priority":'+row.priority
				+',"dicts['+indx+'].systemLevel":'+row.systemLevel+',"dicts['+indx+'].keyword":"'+row.keyword+'","dicts['+indx+'].enabled":'+row.enabled+'}';
			var jsonObj=eval("("+str+')');
			$.extend(items,jsonObj);
			indx++;
		...
		$.each(deleteRows,function(i,row){
			var str='{"dicts['+indx+'].category.remark":"'+$("#remark").val()+'","dicts['+indx+'].category.categoryCode":"'+$("#categoryCode").val()+'","dicts['+indx+'].state":"REMOVE'
				+'","dicts['+indx+'].code":"'+row.code+'","dicts['+indx+'].text":"'+row.text+'","dicts['+indx+'].association":"'+$("#association_code").val()+'","dicts['+indx+'].priority":'+row.priority
				+',"dicts['+indx+'].systemLevel":'+row.systemLevel+',"dicts['+indx+'].keyword":"'+row.keyword+'","dicts['+indx+'].enabled":'+row.enabled+'}';
			var jsonObj=eval("("+str+')');
			$.extend(items,jsonObj);
		...
		if($.isEmptyObject(items)){
			$.doShowMessage("dict.category.item",1);
			return;
		}
		$.ffcsAjax({
			...
			success:function(data){
				if(data){
					$("#add_dict_win").closeWindow();
					$("#dict_grid").jTreegrid("reload",$("#association_code").val());
				}
			}
		});
	});
